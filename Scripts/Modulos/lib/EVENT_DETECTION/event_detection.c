...
char *TAG8 = "EVENT DETECTION TASK";


//-------Event Group Tasks-----------//
EventGroupHandle_t event_detection_event_group;
#define CORR_COEFF BIT0
extern EventGroupHandle_t sampling_signals_event_group;
...
void event_detection_task(void *params)
{
    ...
    event_detection_event_group = xEventGroupCreate();
    ESP_LOGI(TAG8,"-----------------------");
    ESP_LOGI(TAG8,"EVENT DETECTION INITIALIZED");
    ESP_LOGI(TAG8,"-----------------------");
    xEventGroupSetBits(sampling_signals_event_group, START_TIMER);
    while(true){
        EventBits_t bits = xEventGroupWaitBits(event_detection_event_group,
                                                CORR_COEFF,
                                                pdFALSE,
                                                pdFALSE,
                                                portMAX_DELAY);
        ...
        if (bits & CORR_COEFF) {
            xEventGroupClearBits(event_detection_event_group,CORR_COEFF);
